...
{
    /// <summary> Maximum time that a best move test should run for. </summary>
    private const int MaximumSecondsPerTest = 30;

    [TestMethod]
    public void Ecm001()
        => BestMoveTest("2q1r1k1/1ppb4/r2p1Pp1/p4n1p/2P1n3/5NPP/PP3Q1K/2BRRB2 w", "f6", "f7", 2);// Nodes: 1251 

    [TestMethod]
    public void Ecm022()
        => BestMoveTest("r1r3k1/p3qpp1/b1P4p/3p4/3Nn3/4P3/P1Q2PPP/1BR1K2R b K", "e7", "b4", 2);// Nodes: 1,269

    [TestMethod]
    public void Ecm026() => BestMoveTest("r1b5/4k3/p7/3p1n2/3Bp3/2P2r1P/PPBK1P2/4R2R w", "d4", "c5", 2);// Nodes: 917 

    [TestMethod]
    public void Ecm027() => BestMoveTest("r4rk1/1b3Npp/p7/1p3Q2/3P4/1B2q3/P5PP/3n1R1K b", "b7", "g2", 3);// Nodes: 11,298
    ...
    public void Zugzwang_002()
        => BestMoveTest("1q1k4/2Rr4/8/2Q3K1/8/8/8/8 w - - 0 1", "g5", "h6", 5); // Nodes: 37,500
    /*
            [TestMethod]
    ...
    */
    [TestMethod]
    public void Sbd_003() => BestMoveTest("2b1k2r/2p2ppp/1qp4n/7B/1p2P3/5Q2/PPPr2PP/R2N1R1K b k", "e8", "g8", 4);// Nodes: 15,769

    // Failed test ? Was originally commented out.
    //
    //[TestMethod]
    //public void Sbd_004() => BestMoveTest("2b5/1p4k1/p2R2P1/4Np2/1P3Pp1/1r6/5K2/8 w", "d6", "d8", 7);// Nodes: 234,270

    /// <summary>
    /// Helper method that tests that the nest move is found for a known test position, in the required search depth.
    /// </summary>
    private static void BestMoveTest(
        string testPositionFen, string expectedMoveFrom, string expectedMoveTo, int expectedDepth)
    {
        ...
        var brain = game.PlayerToPlay.Brain;
        brain.Think();

        int positions = brain.Search.PositionsSearchedThisTurn;
        Debug.WriteLine(string.Format("Positions: {0} ", positions));
        Debug.WriteLine(string.Format("Nodes: {0} ", brain.Search.PositionsSearchedThisTurn));
        Debug.WriteLine(string.Format("Time Elapsed: {0} ", brain.ThinkingTimeElpased));
        Debug.WriteLine(string.Format("Score: {0} ", brain.PrincipalVariation[0].Score));

        Assert.AreEqual(expectedMoveFrom, brain.PrincipalVariation[0].From.Name, "From move wrong");
        Assert.AreEqual(expectedMoveTo, brain.PrincipalVariation[0].To.Name, "To move wrong");
    }
}