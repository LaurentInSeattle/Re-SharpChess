...
public sealed partial class Evaluation
{
    ...
    /*
    Added dynamic mobility component to 'chillipepper003' from 'mobility13_7e', 
    the same values as in 0.4.3d which have proven best in tests despite a rather high MSE
    MeanSquareError(k=102): 0.23835033815795936
    */

    //Pawn = 0, Knight = 155, Bishop = 305; Rook = 405, Queen = 1050, King = 0
    private static readonly short[] PhaseValues = [0, 155, 305, 405, 1050, 0];

    private static readonly short[] MidgameTables =
    [ 
        //PAWN MG
    ...
    private static readonly short[] EndgameTables =
    [ 
        //PAWN EG
      100,  100,  100,  100,  100,  100,  100,  100,
    ...
    private static readonly short[] MobilityValues =
    [
     //                 opponent                            friendly                  
    ...
    public static int GetMobility(Board board, int square)
    {
        ...
        return piece switch
        {
            Piece.BlackPawn => GetMobility(board, piece, Attacks.BlackPawn[square]),
            Piece.WhitePawn => GetMobility(board, piece, Attacks.WhitePawn[square]),
            Piece.BlackKing or Piece.WhiteKing => GetMobility(board, piece, Attacks.King[square]),
            Piece.BlackKnight or Piece.WhiteKnight => GetMobility(board, piece, Attacks.Knight[square]),
            Piece.BlackRook or Piece.WhiteRook => GetMobilitySlider(board, piece, Attacks.Rook[square]),
            Piece.BlackBishop or Piece.WhiteBishop => GetMobilitySlider(board, piece, Attacks.Bishop[square]),
            Piece.BlackQueen or Piece.WhiteQueen => GetMobilitySlider(board, piece, Attacks.Queen[square]),
            //Piece == Piece.None
            _ => 0,
        };
    }
    ...
    private static int GetMobilitySlider(Board board, Piece piece, byte[][] targets)
    {
        int result = 0;
        int row = 13 * PieceIndex(piece);
        foreach (byte[] axis in targets)
        {
            foreach (byte square in axis)
            {
                Piece targetPiece = board[square];
                int offset = targetPiece.IsColor(piece) ? 6 : 0;
                result += MobilityValues[row + offset + Pieces.Order(targetPiece)];
